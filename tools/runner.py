...
def run_net(args, config, train_writer=None, val_writer=None):
    ...
    best_metrics = None
    metrics = None
    val_freq = config.val_freq if hasattr(config, 'val_freq') else 1
    val_freq = 2
    ...
    for epoch in range(start_epoch, config.max_epoch + 1):
        ...
        if config.model.NAME == "Simba_initial" :
            losses = AverageMeter(['SparseLoss', 'DenseLoss', 'MSELoss']) 
        elif config.model.NAME == "Simba":
            losses = AverageMeter(['SparseLoss', 'DenseLoss', 'MSELoss', 'ProxyLoss'])
        else:
        ...
        for idx, (taxonomy_ids, model_ids, data) in enumerate(train_dataloader):
            ...
            if config.model.NAME=="SymmCompletion":
                ret = base_model(partial)

                loss, sparse_loss, dense_loss, sparse_penalty, dense_penalty = base_model.module.get_loss(ret, gt)
                _loss = loss
            elif config.model.NAME=="SymmGT":
                ret = base_model(partial,gt)

                sparse_loss = base_model.module.get_loss(ret, gt)
                dense_loss = sparse_loss
                _loss = sparse_loss
            elif config.model.NAME=="Simba_initial":
                (rets, denoised_ret, ret) = base_model(partial,gt)
                sparse_loss , fine_loss_1 , fine_loss_2 , mse_loss = base_model.module.get_loss(rets, gt, denoised_ret, ret, epoch)
                dense_loss = fine_loss_2
                _loss = sparse_loss + fine_loss_1 + fine_loss_2 + mse_loss
            elif config.model.NAME=="Simba":
                (rets, denoised_ret, ret, proxy_loss) = base_model(partial,gt)
                sparse_loss , fine_loss_1 , fine_loss_2 , fine_loss_3, mse_loss = base_model.module.get_loss(rets, gt, denoised_ret, ret, epoch)
                dense_loss = fine_loss_3
                _loss = sparse_loss + fine_loss_1 + fine_loss_2 + fine_loss_3 + mse_loss + proxy_loss
            else:
                ret = base_model(partial)
            ...
            if config.model.NAME=="Simba_initial":
                if args.distributed:
                    ...
                    mse_loss = dist_utils.reduce_tensor(mse_loss, args)
                    
                    losses.update([sparse_loss.item() * 1000, dense_loss.item() * 1000, mse_loss.item()])
                else:
                    losses.update([sparse_loss.item() * 1000, dense_loss.item() * 1000, mse_loss.item()])
            elif config.model.NAME=="Simba":
                if args.distributed:
                    ...
                    dense_loss = dist_utils.reduce_tensor(dense_loss, args)
                    mse_loss = dist_utils.reduce_tensor(mse_loss, args)
                    proxy_loss = dist_utils.reduce_tensor(proxy_loss, args)
                    
                    losses.update([sparse_loss.item() * 1000, dense_loss.item() * 1000, mse_loss.item(), proxy_loss.item()])
                else:
                    losses.update([sparse_loss.item() * 1000, dense_loss.item() * 1000, mse_loss.item(), proxy_loss.item()])
            else:
                if args.distributed:
            ...
            if config.model.NAME=="SymmComplete":
                ...
                if train_writer is not None:
                    train_writer.add_scalar('Loss/Batch/Sparse', sparse_loss.item() * 1000, n_itr)
                    train_writer.add_scalar('Loss/Batch/Dense', dense_loss.item() * 1000, n_itr)
                    train_writer.add_scalar('Loss/Batch/MSE', mse_loss.item() * 1000, n_itr)
            elif config.model.NAME=="Simba" :
                if train_writer is not None:
                    train_writer.add_scalar('Loss/Batch/Sparse', sparse_loss.item() * 1000, n_itr)
                    train_writer.add_scalar('Loss/Batch/Dense', dense_loss.item() * 1000, n_itr)
                    train_writer.add_scalar('Loss/Batch/MSE', mse_loss.item() * 1000, n_itr)
                    train_writer.add_scalar('Loss/Batch/Proxy', proxy_loss.item() * 1000, n_itr)
            else:
                if train_writer is not None:
                    train_writer.add_scalar('Loss/Batch/Sparse', sparse_loss.item() * 1000, n_itr)
                    train_writer.add_scalar('Loss/Batch/Dense', dense_loss.item() * 1000, n_itr)
        ...
        if config.model.NAME=="SymmComplete":
            ...
            if train_writer is not None:
                train_writer.add_scalar('Loss/Epoch/Sparse', losses.avg(0), epoch)
                train_writer.add_scalar('Loss/Epoch/Dense', losses.avg(1), epoch)
                train_writer.add_scalar('Loss/Epoch/MSE', losses.avg(2), epoch)
        elif config.model.NAME=="Simba" :
            if train_writer is not None:
                train_writer.add_scalar('Loss/Epoch/Sparse', losses.avg(0), epoch)
                train_writer.add_scalar('Loss/Epoch/Dense', losses.avg(1), epoch)
                train_writer.add_scalar('Loss/Epoch/MSE', losses.avg(2), epoch)
                train_writer.add_scalar('Loss/Epoch/Proxy', losses.avg(3), epoch)
        else:
            if train_writer is not None:
                train_writer.add_scalar('Loss/Epoch/Sparse', losses.avg(0), epoch)
                train_writer.add_scalar('Loss/Epoch/Dense', losses.avg(1), epoch)
...
def validate(base_model, test_dataloader, epoch, ChamferDisL1, ChamferDisL2, val_writer, args, config, logger = None):
    ...
    with torch.no_grad():
        for idx, (taxonomy_ids, model_ids, data) in enumerate(test_dataloader):
            ...
            if config.model.NAME=="PrePccp" or config.model.NAME=="SymmGT":
                ret = base_model(partial,gt)
            else:
                ret = base_model(partial)
            coarse_points = ret[0]
