...
def dataset_builder(args, config):
    """Build dataset and dataloader based on configuration"""
...
def build_optimizer(base_model, config):
    """Build optimizer with weight decay configuration"""
...
def build_scheduler(base_model, optimizer, config, last_epoch=-1):
    ...
    if sche_config.type == 'LambdaLR':
        ...
        assert sche_config.kwargs.lr_max == config.optimizer.kwargs.lr
        scheduler = build_warm_cos_sche(optimizer, sche_config.kwargs, last_epoch=last_epoch)  # misc.py
    ...
    # Build batch normalization scheduler if configured
...
def resume_model(base_model, args, logger = None):
    ...
    # Filter out compilation-related keys
    filtered_ckpt = {}
    for key, value in base_ckpt.items():
        # Skip compilation-related keys
        if '_compiled_denoise._orig_mod.' in key:
            # Convert to original key name
            new_key = key.replace('_compiled_denoise._orig_mod.', '')
            filtered_ckpt[new_key] = value
        elif not key.startswith('_compiled'):
...
def load_model(base_model, ckpt_path, logger = None):
    ...
    # Extract epoch and metrics information
    epoch = -1
